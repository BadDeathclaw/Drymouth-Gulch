...
/obj/item/rig_module/device/Initialize()
	...
	if(device_type)
		...
		device.item_flags |= ABSTRACT //Abstract in the sense that it's not an item that stands alone, but rather is just there to let the module act like it.
...
/obj/item/rig_module/welding_tank/engage(atom/target)
	...
	if(!target)
		if(get_fuel() >= 0)
			var/obj/item/weldingtool/W = holder.wearer.get_active_held_item()
			if(istype(W))
				fill_welder(W)
			else
