...
/obj/item/gun
	...
	var/muzzle_speed_multiplier = 1		//Number to multiply the projectile speed by. Lower = faster
...
/obj/item/gun/proc/process_burst(mob/living/user, atom/target, message = TRUE, params=null, zone_override = "", sprd = 0, randomized_gun_spread = 0, randomized_bonus_spread = 0, rand_spr = 0, iteration = 0)
	...
	if(chambered && chambered.BB)
		...
		if(!chambered.fire_casing(target, user, params, distro,suppressed, zone_override, sprd, extra_damage, extra_penetration, muzzle_speed_multiplier))
			shoot_with_empty_chamber(user)
			firing_burst = FALSE
			return FALSE
		else
