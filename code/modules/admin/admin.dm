...
/proc/message_admins(msg)
	msg = "<span class=\"admin\"><span class=\"prefix\">[time_stamp()] ADMIN LOG:</span> <span class=\"message\">[msg]</span></span>"
	to_chat(GLOB.admins, msg)

/proc/relay_msg_admins(msg)
	msg = "<span class=\"admin\"><span class=\"prefix\">[time_stamp()] RELAY:</span> <span class=\"message\">[msg]</span></span>"
	to_chat(GLOB.admins, msg)
...
/datum/admins/proc/access_news_network() //MARKER
	...
	switch(admincaster_screen)
		if(0)
			...
			dat+="<BR><A href='?src=[REF(src)];[HrefToken()];ac_menu_censor_channel=1'>Mark Feed Channel with D-Notice (disables and locks the channel).</A>"
		...
		if(9)
			...
			if(src.admincaster_feed_channel.censored)
				dat+="<FONT COLOR='red'><B>ATTENTION: </B></FONT>This channel has been deemed as threatening to the welfare of the station, and marked with a D-Notice.<BR>"
