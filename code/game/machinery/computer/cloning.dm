...
/obj/machinery/computer/cloning/proc/scan_occupant(occupant)
	...
	if(!LAZYLEN(pods))
		...
		playsound(src, 'sound/machines/terminal_prompt_deny.ogg', 50, 0)
	else if(pod.growclone(mob_occupant.ckey, mob_occupant.real_name, dna.uni_identity, dna.struc_enzymes, mind, mrace, dna.features, mob_occupant.faction, quirks,))
		temp = "[mob_occupant.real_name] => <font class='good'>Cloning cycle in progress...</font>"
		playsound(src, 'sound/machines/terminal_prompt_confirm.ogg', 50, 0)
	else
		temp = "[mob_occupant.real_name] => <font class='bad'>Initialisation failure. Subject still alive.</font>"
		playsound(src, 'sound/machines/terminal_prompt_deny.ogg', 50, 0)